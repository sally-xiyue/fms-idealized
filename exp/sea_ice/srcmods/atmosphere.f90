...
use    spectral_dynamics_mod, only: spectral_dynamics_init, spectral_dynamics_outputtend, spectral_dynamics_end, &  ! spectral_dynamics_outputtend: ZTAN renewed
...
use          diffusivity_mod, only: diffusivity_init !!don't know whether it is needed
...
real, allocatable, dimension(:,:,:    ) :: psg, wg_full, wg ! wg added by ZTAN 01/22/2013
...
real, allocatable, dimension(:,:,:) ::                                        &
     ...
     rad_dt_tg,            &   ! temperature tendency from radiation, by ZTAN
! Added by ZTAN 09/11/2012 for diagnosis of T and q tendencies from the spectral_dynamics
...
! Added by ZTAN 10/16/2012: ps-weighted values
     ...
     id_dt_tg_param_ps, id_dt_qg_param_ps, id_dt_tg_fino_ps,  id_dt_tg_hadv_ps,  id_dt_tg_vadv_ps,  id_dt_qg_hadv_ps, id_dt_qg_vadv_ps,&
     id_dt_tg_total_ps, id_dt_qg_total_ps, id_dt_tg_real1_ps, id_dt_qg_real1_ps, id_dt_tg_real2_ps, id_dt_qg_real2_ps, &
! Added by ZTAN 11/12/2012: sqr values for computation of variance
...
allocate (wg           (is:ie, js:je, num_levels+1))   ! wg added by ZTAN 01/22/2013
...
allocate(rad_dt_tg   (is:ie, js:je, num_levels))  ! added ZTAN 02/06/2012

! Added by ZTAN 09/11/2012 for diagnosis of T and q from the spectral_dynamics
allocate(dt_tg_param (is:ie, js:je, num_levels))
allocate(dt_qg_param (is:ie, js:je, num_levels))
allocate(dt_tg_fino  (is:ie, js:je, num_levels))
allocate(dt_tg_hadv  (is:ie, js:je, num_levels))
allocate(dt_tg_vadv  (is:ie, js:je, num_levels))
allocate(dt_qg_hadv  (is:ie, js:je, num_levels))
allocate(dt_qg_vadv  (is:ie, js:je, num_levels))
allocate(dt_tg_total (is:ie, js:je, num_levels))
allocate(dt_qg_total (is:ie, js:je, num_levels))
allocate(dt_tg_real1 (is:ie, js:je, num_levels))
allocate(dt_qg_real1 (is:ie, js:je, num_levels))
allocate(dt_tg_real2 (is:ie, js:je, num_levels))
allocate(dt_qg_real2 (is:ie, js:je, num_levels))
! End of addition by ZTAN 09/11/2012 for diagnosis of T and q from the spectral_dynamics

! Added by ZTAN 09/07/2013 for diagnosis of u and v from the spectral_dynamics
allocate(dt_ug_fino (is:ie, js:je, num_levels))
allocate(dt_vg_fino (is:ie, js:je, num_levels))
allocate(dt_ug_vadv (is:ie, js:je, num_levels))
allocate(dt_vg_vadv (is:ie, js:je, num_levels))
allocate(dt_ug_hadv (is:ie, js:je, num_levels))
allocate(dt_vg_hadv (is:ie, js:je, num_levels))
allocate(dt_ug_pres (is:ie, js:je, num_levels))
allocate(dt_vg_pres (is:ie, js:je, num_levels))
allocate(dt_ug_cori (is:ie, js:je, num_levels))
allocate(dt_vg_cori (is:ie, js:je, num_levels))
allocate(dt_ug_total (is:ie, js:je, num_levels))
allocate(dt_vg_total (is:ie, js:je, num_levels))
allocate(dt_ug_real1 (is:ie, js:je, num_levels))
allocate(dt_vg_real1 (is:ie, js:je, num_levels))
allocate(dt_ug_real2 (is:ie, js:je, num_levels))
allocate(dt_vg_real2 (is:ie, js:je, num_levels))
! End of addition by ZTAN 09/07/2013 for diagnosis of u and v from the spectral_dynamics
...
wg = 0.  ! wg added by ZTAN 01/22/2013
...
! Added by ZTAN: DIAGNOSIS of TENDENCIES (T and q) 09/11/2012
   id_dt_tg_param = register_diag_field(mod_name, 'dt_tg_param',        &
        axes(1:3), Time, 'temperature tendency: by all parameterization','K/s')
   id_dt_qg_param = register_diag_field(mod_name, 'dt_qg_param',        &
        axes(1:3), Time, 'moisture tendency: by all parameterization','1/s')
   id_dt_tg_fino  = register_diag_field(mod_name, 'dt_tg_fino',        &
        axes(1:3), Time, 'temperature tendency: by Four-in-One computation','K/s')
   id_dt_tg_hadv  = register_diag_field(mod_name, 'dt_tg_hadv',        &
        axes(1:3), Time, 'temperature tendency: by horizontal advection','K/s')
   id_dt_tg_vadv  = register_diag_field(mod_name, 'dt_tg_vadv',        &
        axes(1:3), Time, 'temperature tendency: by vertical advection','K/s')
   id_dt_qg_hadv  = register_diag_field(mod_name, 'dt_qg_hadv',        &
        axes(1:3), Time, 'moisture tendency: by horizontal advection','1/s')
   id_dt_qg_vadv  = register_diag_field(mod_name, 'dt_qg_vadv',        &
        axes(1:3), Time, 'moisture tendency: by vertical advection','1/s')
   id_dt_tg_total = register_diag_field(mod_name, 'dt_tg_total',        &
        axes(1:3), Time, 'temperature tendency: EVERYTHING except spectral damping and conservation corrections','K/s')
   id_dt_qg_total  = register_diag_field(mod_name, 'dt_qg_total',        &
        axes(1:3), Time, 'moisture tendency: EVERYTHING except spectral damping and conservation corrections','1/s')
   id_dt_tg_real1  = register_diag_field(mod_name, 'dt_tg_real1',        &
        axes(1:3), Time, 'temperature tendency: EVERYTHING INCLUDING spectral damping and conservation corrections','K/s')
   id_dt_qg_real1  = register_diag_field(mod_name, 'dt_qg_real1',        &
        axes(1:3), Time, 'moisture tendency: EVERYTHING INCLUDING spectral damping and conservation corrections','1/s')
   id_dt_tg_real2  = register_diag_field(mod_name, 'dt_tg_real2',        &
        axes(1:3), Time, 'temperature tendency: Adjustment by 2nd step of RAW filter','K/s')
   id_dt_qg_real2  = register_diag_field(mod_name, 'dt_qg_real2',        &
        axes(1:3), Time, 'moisture tendency: Adjustment by 2nd step of RAW filter','1/s')
! End of Addition by ZTAN: DIAGNOSIS of TENDENCIES (T and q) 09/11/2012

! Added by ZTAN: DIAGNOSIS of TENDENCIES (u and v) 09/07/2013
   id_dt_ug_fino  = register_diag_field(mod_name, 'dt_ug_fino',        &
        axes(1:3), Time, 'U tendency: by Four-in-One computation','m/s^2')
   id_dt_vg_fino  = register_diag_field(mod_name, 'dt_vg_fino',        &
        axes(1:3), Time, 'V tendency: by Four-in-One computation','m/s^2')
   id_dt_ug_hadv  = register_diag_field(mod_name, 'dt_ug_hadv',        &
        axes(1:3), Time, 'U tendency: by horizontal advection','m/s^2')
   id_dt_vg_hadv  = register_diag_field(mod_name, 'dt_vg_hadv',        &
        axes(1:3), Time, 'V tendency: by horizontal advection','m/s^2')
   id_dt_ug_vadv  = register_diag_field(mod_name, 'dt_ug_vadv',        &
        axes(1:3), Time, 'U tendency: by vertical advection','m/s^2')
   id_dt_vg_vadv  = register_diag_field(mod_name, 'dt_vg_vadv',        &
        axes(1:3), Time, 'V tendency: by vertical advection','m/s^2')
   id_dt_ug_pres  = register_diag_field(mod_name, 'dt_ug_pres',        &
        axes(1:3), Time, 'U tendency: by pressure gradient','m/s^2')
   id_dt_vg_pres  = register_diag_field(mod_name, 'dt_vg_pres',        &
        axes(1:3), Time, 'V tendency: by pressure gradient','m/s^2')
   id_dt_ug_cori  = register_diag_field(mod_name, 'dt_ug_cori',        &
        axes(1:3), Time, 'U tendency: by Coriolis acceleration','m/s^2')
   id_dt_vg_cori  = register_diag_field(mod_name, 'dt_vg_cori',        &
        axes(1:3), Time, 'V tendency: by Coriolis acceleration','m/s^2')
   id_dt_ug_total  = register_diag_field(mod_name, 'dt_ug_total',        &
        axes(1:3), Time, 'U tendency: EVERYTHING except spectral damping and conservation corrections','m/s^2')
   id_dt_vg_total  = register_diag_field(mod_name, 'dt_vg_total',        &
        axes(1:3), Time, 'V tendency: EVERYTHING except spectral damping and conservation corrections','m/s^2')
   id_dt_ug_real1  = register_diag_field(mod_name, 'dt_ug_real1',        &
        axes(1:3), Time, 'U tendency: EVERYTHING INCLUDING spectral damping and conservation corrections','m/s^2')
   id_dt_vg_real1  = register_diag_field(mod_name, 'dt_vg_real1',        &
        axes(1:3), Time, 'V tendency: EVERYTHING INCLUDING spectral damping and conservation corrections','m/s^2')
   id_dt_ug_real2  = register_diag_field(mod_name, 'dt_ug_real2',        &
        axes(1:3), Time, 'U tendency: Adjustment by 2nd step of RAW filter','m/s^2')
   id_dt_vg_real2  = register_diag_field(mod_name, 'dt_vg_real2',        &
        axes(1:3), Time, 'V tendency: Adjustment by 2nd step of RAW filter','m/s^2')
! End of Addition by ZTAN: DIAGNOSIS of TENDENCIES (u and v) 09/07/2013


! Added by ZTAN: DIAGNOSIS of ps-weighted Tendencies: 10/16/2012
   id_dt_tg_param_ps = register_diag_field(mod_name, 'dt_tg_param_ps',        &
        axes(1:3), Time, 'temperature tendency: by all parameterization multipled by ps','K/s*Pa')
   id_dt_qg_param_ps = register_diag_field(mod_name, 'dt_qg_param_ps',        &
        axes(1:3), Time, 'moisture tendency: by all parameterization multipled by ps','1/s*Pa')
   id_dt_tg_fino_ps  = register_diag_field(mod_name, 'dt_tg_fino_ps',        &
        axes(1:3), Time, 'temperature tendency: by Four-in-One computation multipled by ps','K/s*Pa')
   id_dt_tg_hadv_ps  = register_diag_field(mod_name, 'dt_tg_hadv_ps',        &
        axes(1:3), Time, 'temperature tendency: by horizontal advection multipled by ps','K/s*Pa')
   id_dt_tg_vadv_ps  = register_diag_field(mod_name, 'dt_tg_vadv_ps',        &
        axes(1:3), Time, 'temperature tendency: by vertical advection multipled by ps','K/s*Pa')
   id_dt_qg_hadv_ps  = register_diag_field(mod_name, 'dt_qg_hadv_ps',        &
        axes(1:3), Time, 'moisture tendency: by horizontal advection multipled by ps','1/s*Pa')
   id_dt_qg_vadv_ps  = register_diag_field(mod_name, 'dt_qg_vadv_ps',        &
        axes(1:3), Time, 'moisture tendency: by vertical advection multipled by ps','1/s*Pa')
   id_dt_tg_total_ps = register_diag_field(mod_name, 'dt_tg_total_ps',        &
        axes(1:3), Time, 'temperature tendency: EVERYTHING except spec_dyn multipled by ps','K/s*Pa')
   id_dt_qg_total_ps  = register_diag_field(mod_name, 'dt_qg_total_ps',        &
        axes(1:3), Time, 'moisture tendency: EVERYTHING INCLUDING spec_dyn multipled by ps','1/s*Pa')
   id_dt_tg_real1_ps  = register_diag_field(mod_name, 'dt_tg_real1_ps',        &
        axes(1:3), Time, 'temperature tendency: EVERYTHING INCLUDING spec_dyn multipled by ps','K/s*Pa')
   id_dt_qg_real1_ps  = register_diag_field(mod_name, 'dt_qg_real1_ps',        &
        axes(1:3), Time, 'moisture tendency: EVERYTHING INCLUDING spec_dyn multipled by ps','1/s*Pa')
   id_dt_tg_real2_ps  = register_diag_field(mod_name, 'dt_tg_real2_ps',        &
        axes(1:3), Time, 'temperature tendency: Adjustment by 2nd step of RAW filter multipled by ps','K/s*Pa')
   id_dt_qg_real2_ps  = register_diag_field(mod_name, 'dt_qg_real2_ps',        &
        axes(1:3), Time, 'moisture tendency: Adjustment by 2nd step of RAW filter multipled by ps','1/s*Pa')

if(.not.dry_model) then
   id_cond_dt_qg_ps = register_diag_field(mod_name, 'dt_qg_cond_ps',        &
        axes(1:3), Time, 'Moisture tendency from condensation multipled by ps','kg/kg/s*Pa')
   id_cond_dt_tg_ps = register_diag_field(mod_name, 'dt_tg_cond_ps',        &
        axes(1:3), Time, 'Temperature tendency from condensation multipled by ps','K/s*Pa')
endif
if(lwet_convection) then
   id_conv_dt_qg_ps = register_diag_field(mod_name, 'dt_qg_conv_ps',        &
...
! Added by ZTAN: DIAGNOSIS of variance of tendencies: 11/12/2012
   id_dt_tg_fino_sqr_ps  = register_diag_field(mod_name, 'dt_tg_fino_sqr_ps',        &
        axes(1:3), Time, 'Squared temperature tendency: by Four-in-One computation multipled by ps','10^(-6)K^2/s^2*Pa')
   id_dt_tg_hadv_sqr_ps  = register_diag_field(mod_name, 'dt_tg_hadv_sqr_ps',        &
        axes(1:3), Time, 'Squared temperature tendency: by horizontal advection multipled by ps','10^(-6)K^2/s^2*Pa')
...
if(.not.dry_model) then
   id_cond_dt_tg_sqr_ps = register_diag_field(mod_name, 'dt_tg_cond_sqr_ps',        &
        axes(1:3), Time, 'Squared temperature tendency: by condensation multipled by ps','10^(-6)K^2/s^2*Pa')
   id_cond_dt_qg_sqr_ps = register_diag_field(mod_name, 'dt_qg_cond_sqr_ps',        &
        axes(1:3), Time, 'Squared moisture tendency: by condensation multipled by ps','10^(-12)/s^2*Pa')
endif
if(lwet_convection) then
   id_conv_dt_tg_sqr_ps = register_diag_field(mod_name, 'dt_tg_conv_sqr_ps',        &
        axes(1:3), Time, 'Squared temperature tendency: by convection multipled by ps','10^(-6)K^2/s^2*Pa')
   id_conv_dt_qg_sqr_ps = register_diag_field(mod_name, 'dt_qg_conv_sqr_ps',        &
        axes(1:3), Time, 'Squared moisture tendency: by convection multipled by ps','10^(-12)/s^2*Pa')
endif
if(two_stream) then
   id_rad_dt_tg_sqr_ps   = register_diag_field(mod_name, 'dt_tg_rad_sqr_ps',        &
        axes(1:3), Time, 'Squared temperature tendency: by radiation multipled by ps','10^(-6)K^2/s^2*Pa')
endif

! End of ZTAN Addidtion: DIAGNOSIS of TENDENCIES  09/11/2012
...
rad_dt_tg    = 0.0 ! Added ZTAN 02/06/2012
...
call surface_flux(                                                          &
   ...
   if(id_qstar  > 0) used = send_data(id_qstar,  qstar,  Time)

! Now complete the radiation calculation by computing the upward and net fluxes.

if(two_stream) then
   rad_dt_tg = dt_tg    ! use rad_dt_tg to store previous dt_tg temporarily
...
call spectral_dynamics_outputtend(Time, psg(:,:,future), ug(:,:,:,future), vg(:,:,:,future), &
                       tg(:,:,:,future), tracer_attributes, grid_tracers(:,:,:,:,:), future, &
                       dt_psg, dt_ug, dt_vg, dt_tg, dt_tracers, wg_full, wg, p_full, p_half, z_full, & ! wg added by ZTAN 01/22/2013
                       dt_tg_param, dt_qg_param, dt_tg_fino, dt_tg_hadv, dt_tg_vadv, dt_qg_hadv, dt_qg_vadv, &
...
! Added by ZTAN: DIAGNOSIS of TENDENCIES (T,q) 09/11/2012
   if(id_dt_tg_param > 0)  used = send_data(id_dt_tg_param,  dt_tg_param, Time)
   if(id_dt_qg_param > 0)  used = send_data(id_dt_qg_param,  dt_qg_param, Time)
   if(id_dt_tg_fino  > 0)  used = send_data(id_dt_tg_fino,   dt_tg_fino,  Time)
   if(id_dt_tg_hadv  > 0)  used = send_data(id_dt_tg_hadv,   dt_tg_hadv,  Time)
   if(id_dt_tg_vadv  > 0)  used = send_data(id_dt_tg_vadv,   dt_tg_vadv,  Time)
   if(id_dt_qg_hadv  > 0)  used = send_data(id_dt_qg_hadv,   dt_qg_hadv,  Time)
   if(id_dt_qg_vadv  > 0)  used = send_data(id_dt_qg_vadv,   dt_qg_vadv,  Time)
   if(id_dt_tg_total > 0)  used = send_data(id_dt_tg_total,  dt_tg_total, Time)
   if(id_dt_qg_total > 0)  used = send_data(id_dt_qg_total,  dt_qg_total, Time)
   if(id_dt_tg_real1 > 0)  used = send_data(id_dt_tg_real1,  dt_tg_real1, Time)
   if(id_dt_qg_real1 > 0)  used = send_data(id_dt_qg_real1,  dt_qg_real1, Time)
   if(id_dt_tg_real2 > 0)  used = send_data(id_dt_tg_real2,  dt_tg_real2, Time)
...
!   if(id_dt_vg_param > 0)  used = send_data(id_dt_vg_param,  dt_vg_param, Time)
   ...
   if(id_dt_vg_fino  > 0)  used = send_data(id_dt_vg_fino,   dt_vg_fino,  Time)
   if(id_dt_ug_hadv  > 0)  used = send_data(id_dt_ug_hadv,   dt_ug_hadv,  Time)
   if(id_dt_vg_hadv  > 0)  used = send_data(id_dt_vg_hadv,   dt_vg_hadv,  Time)
   if(id_dt_ug_vadv  > 0)  used = send_data(id_dt_ug_vadv,   dt_ug_vadv,  Time)
   if(id_dt_vg_vadv  > 0)  used = send_data(id_dt_vg_vadv,   dt_vg_vadv,  Time)
   if(id_dt_ug_pres  > 0)  used = send_data(id_dt_ug_pres,   dt_ug_pres,  Time)
   if(id_dt_vg_pres  > 0)  used = send_data(id_dt_vg_pres,   dt_vg_pres,  Time)
   if(id_dt_ug_cori  > 0)  used = send_data(id_dt_ug_cori,   dt_ug_cori,  Time)
   if(id_dt_vg_cori  > 0)  used = send_data(id_dt_vg_cori,   dt_vg_cori,  Time)
   if(id_dt_ug_total > 0)  used = send_data(id_dt_ug_total,  dt_ug_total, Time)
   if(id_dt_vg_total > 0)  used = send_data(id_dt_vg_total,  dt_vg_total, Time)
   if(id_dt_ug_real1 > 0)  used = send_data(id_dt_ug_real1,  dt_ug_real1, Time)
   if(id_dt_vg_real1 > 0)  used = send_data(id_dt_vg_real1,  dt_vg_real1, Time)
   if(id_dt_ug_real2 > 0)  used = send_data(id_dt_ug_real2,  dt_ug_real2, Time)
   if(id_dt_vg_real2 > 0)  used = send_data(id_dt_vg_real2,  dt_vg_real2, Time)
! End of ZTAN Addition: DIAGNOSIS of TENDENCIES  09/07/2013


! Added ZTAN: ps weighted values: 10162012
   if(id_rad_dt_tg_ps   > 0)   then
      call  compute_ps_wt_value (rad_dt_tg, psg(:,:,future) , ps_wt_value)
      used = send_data(id_rad_dt_tg_ps,   ps_wt_value, Time)
   end if

   if(id_cond_dt_tg_ps   > 0)   then
      call  compute_ps_wt_value (cond_dt_tg, psg(:,:,future) , ps_wt_value)
      used = send_data(id_cond_dt_tg_ps,   ps_wt_value, Time)
   end if
   if(id_cond_dt_qg_ps   > 0)   then
      call  compute_ps_wt_value (cond_dt_qg, psg(:,:,future) , ps_wt_value)
      used = send_data(id_cond_dt_qg_ps,   ps_wt_value, Time)
   end if
   if(id_conv_dt_tg_ps   > 0)   then
      call  compute_ps_wt_value (conv_dt_tg, psg(:,:,future) , ps_wt_value)
      used = send_data(id_conv_dt_tg_ps,   ps_wt_value, Time)
   end if
   if(id_conv_dt_qg_ps   > 0)   then
   ...
   if(id_dt_qg_param_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_param, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_param_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_fino_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_fino, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_fino_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_hadv_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_hadv, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_hadv_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_hadv_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_hadv, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_hadv_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_vadv_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_vadv, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_vadv_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_vadv_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_vadv, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_vadv_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_total_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_total, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_total_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_total_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_total, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_total_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_real1_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_real1, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_real1_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_real1_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_real1, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_real1_ps,   ps_wt_value, Time)
   end if

   if(id_dt_tg_real2_ps   > 0)   then
      call  compute_ps_wt_value (dt_tg_real2, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_tg_real2_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_real2_ps   > 0)   then
      call  compute_ps_wt_value (dt_qg_real2, psg(:,:,future) , ps_wt_value)
      used = send_data(id_dt_qg_real2_ps,   ps_wt_value, Time)
   end if

! End of ZTAN Addition: ps weighted values: 10162012

! Begin  ZTAN Addition: ps weighted variances: 11122012
   if(id_cond_dt_tg_sqr_ps  > 0)   then
      call  compute_ps_wt_value (cond_dt_tg*cond_dt_tg*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_cond_dt_tg_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_cond_dt_qg_sqr_ps  > 0)   then
      call  compute_ps_wt_value (cond_dt_qg*cond_dt_qg*1.0e12, psg(:,:,future) , ps_wt_value)
      used = send_data( id_cond_dt_qg_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_conv_dt_tg_sqr_ps  > 0)   then
      call  compute_ps_wt_value (conv_dt_tg*conv_dt_tg*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_conv_dt_tg_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_conv_dt_qg_sqr_ps  > 0)   then
      call  compute_ps_wt_value (conv_dt_qg*conv_dt_qg*1.0e12, psg(:,:,future) , ps_wt_value)
      used = send_data( id_conv_dt_qg_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_rad_dt_tg_sqr_ps  > 0)   then
      call  compute_ps_wt_value (rad_dt_tg*rad_dt_tg*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_rad_dt_tg_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_tg_fino_sqr_ps  > 0)   then
      call  compute_ps_wt_value (dt_tg_fino*dt_tg_fino*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_tg_fino_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_tg_hadv_sqr_ps  > 0)   then
      call  compute_ps_wt_value (dt_tg_hadv*dt_tg_hadv*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_tg_hadv_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_tg_vadv_sqr_ps  > 0)   then
      call  compute_ps_wt_value (dt_tg_vadv*dt_tg_vadv*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_tg_vadv_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_hadv_sqr_ps  > 0)   then
      call  compute_ps_wt_value (dt_qg_hadv*dt_qg_hadv*1.0e12, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_qg_hadv_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_qg_vadv_sqr_ps  > 0)   then
      call  compute_ps_wt_value (dt_qg_vadv*dt_qg_vadv*1.0e12, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_qg_vadv_sqr_ps,   ps_wt_value, Time)
   end if
   if(id_dt_tg_vadvtot_sqr_ps > 0)   then
      call  compute_ps_wt_value ((dt_tg_vadv+dt_tg_fino)*(dt_tg_vadv+dt_tg_fino)*1.0e6, psg(:,:,future) , ps_wt_value)
      used = send_data( id_dt_tg_vadvtot_sqr_ps,   ps_wt_value, Time)
   end if
! End of ZTAN Addition: ps weighted values: 11122012


! End of Addition by ZTAN: DIAGNOSIS of TENDENCIES  09/11/2012
